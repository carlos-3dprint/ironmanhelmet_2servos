...
const byte openLED = 11;//pin for the LED
const byte closeLED = 12;//pin for the LED
...
void loop() {
  ...
  if ( interruptFlag == true) {
    ...
    for (int i = 40; i < 160; i++) {//IMPORTANT! Perform some tests with these values (40 and 160) before installing the mechanics
      facePlateServo.write(i);
      facePlateServo2.write(180 - i); //mirrored servo
      delay (5);
    }//close For loop
    
  // CHANGED ON 02/27/2022
    facePlateServo.detach();//detach servo 1 to avoid mechanical problems
    facePlateServo2.detach();//detach servo 2 to avoid mechanical problems
     
    delay(700);//Wait to launch the blink sequence
  ...
  // OPEN FACEPLATE
    ...
    for (int i = 160; i > 60; i--) {//IMPORTANT! Perform some tests with these values (40 and 160) before installing the mechanics
      facePlateServo.write(i);
      facePlateServo2.write(180 - i);//mirrored servo
      delay (5);
    }//close For loop
